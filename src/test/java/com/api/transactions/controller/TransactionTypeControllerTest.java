...
class TransactionTypeControllerTest {
    ...
    void when_listTransactionTypes_returnSuccess() throws Exception {
        when(transactionTypeService.listTransactionTypes()).thenReturn(List.of(MockTransactionType.mockDebitTransactionType()));

        mockMvc.perform(get(URL_TRANSACTION_TYPE))
                .andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(List.of(MockTransactionType.mockDebitTransactionType()))));
